...
class Data extends BasicAdmin
{
    ...
    public function export($ids = null, $id = null, $start = null)
    {
        ...
        if ($Request->isPost() && !empty($ids)) { //初始化
            ...
            $ids = explode(',', $ids);
            $path = config('backups.data_backup_path');
            is_dir($path) || mkdir($path, 0755, true);
            //读取备份配置
            $config = [
                'path' => realpath($path) . DIRECTORY_SEPARATOR,
                'part' => config('backups.data_backup_part_size'),
                'compress' => config('backups.data_backup_compress'),
                'level' => config('backups.data_backup_compress_level'),
            ];
            ...
            if (false !== $Database->create()) {
                $tab = ['id' => 0, 'start' => 0];
                $this->success('初始化成功！', '', ['tables' => $ids, 'tab' => $tab]);
            } else {
    ...
    {
        //列出备份文件列表
        $path_tmp = config('backups.data_backup_path');
    ...
    public function del($time = 0)
    {
        ...
        $path = realpath(config('backups.data_backup_path')) . DIRECTORY_SEPARATOR . $name;
    ...
    public function revert($time = 0, $part = null, $start = null)
    {
        if (is_numeric($time) && is_null($part) && is_null($start)) { //初始化
            ...
            if (count($list) === $last[0]) {
                ...
                $this->success('初始化完成,请等待！', '', ['part' => 1, 'start' => 0]);
            } else {
            ...
            $db = new \com\Database($list[$part], [
                    'path' => realpath(config('backups.data_backup_path')) . DIRECTORY_SEPARATOR,
