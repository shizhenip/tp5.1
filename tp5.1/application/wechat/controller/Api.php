...
class Api extends Controller {
        ...
        if (is_array($info = Db::name($table)->where($field, $value)->find()) && isset($info['type'])) {
            ...
            switch ($info['type']) {
                ...
                case 'text': // 文本消息
                    ...
                    if($info['name'] =="手机绑定"){
                      //  $wechathao=$this->wechat->getRevTo();//openid

                        $openid=$this->wechat->getRevFrom();//用户名

                        $content[] = array("Title"=>"手机绑定", "Description"=>"点击菜单绑定手机，然后进入页面，输入手机号和密码进行绑定，绑定成功以后点击查课表可以自助查询课表,并接受课程通知" ,"PicUrl"=>"", "Url" =>"http://fx.jooins.com/bindmobile/?openid=".$openid);
    ...
    protected function _event() {
        ...
        switch (strtolower($event['event'])) {
            ...
            case 'click': // 点击菜单事件
                //$enventKey=$this
               // $aa=json_encode($event);
                //$openid=$this->wechat->getRevTo();//openid
               // $FromUserName=$this->wechat->getRevFrom();//用户名
              //  return $this->wechat->text($aa)->reply();
