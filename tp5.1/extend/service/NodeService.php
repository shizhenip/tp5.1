...
class NodeService
{
    ...
    public static function applyAuthNode()
    {
        ...
        if (($authorize = session('user.authorize'))) {
            $authorizeids = Db::name('SystemAuth')->whereIn('id', explode(',', $authorize))->where('status', '1')->column('id');
    ...
    {
        $alias = [];
        foreach (Db::name('SystemNode')->cache('getSystemNode', 60)->select() as $vo) {
            $alias["{$vo['node']}"] = $vo;
        }
        $nodes = [];
        $ignore = [
            'index', 'admin/login', 'wechat/api', 'wechat/notify',
            'wechat/review', 'admin/index', 'admin/plugs'
        ];
        foreach (self::getNodeTree(Env::get('app_path')) as $thr) {
    ...
    public static function getNodeTree($path, $nodes = [])
    {
        foreach (self::_getFilePaths($path) as $vo) {
            if (!preg_match('|/(\w+)/controller/(\w+)|', str_replace(DIRECTORY_SEPARATOR, '/', $vo), $matches) || count($matches) !== 3) {
                continue;
            }
